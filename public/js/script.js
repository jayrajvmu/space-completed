function display() {
    ...
    if (select == 'WS101') {
      // if(select =)
      axios.get("http://127.0.0.1:5000/availability/wing/1")
        .then(response => {
          ...
          colTable.map(item => {
            let tableCol = show.innerHTML += `<div class ="table" id="table-${item.id}"></div>`;
            item.seats.map(seat => {
              if (document.querySelector(`#table-${item.id}`)) {
  
                document.querySelector(`#table-${item.id}`).innerHTML += `<div class="chair" id=${seat.seatsId} ></div>`
              }
    ...
    let option = document.querySelector('#wing').value;
    if (select == 'WS102') {
      axios.get("http://127.0.0.1:5000/availability/wing/2")
        .then(response => {
          // let res = response.data.availability[0].wings[0].tables[0].seats;
          let res = response.data.availability[0].wings[0].tables[0].seats;
