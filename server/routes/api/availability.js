  ...
  let query = connection.query(sql, (err, result, fields) => {
    ...
    for (let j = 0; j < result.length; j++) {
      ...
      tables_data.push({
        ...
        tablename: result[j].tname,
        tableswings: result[j].twings,
      });
      seats_data.push({
        date: result[j].date,
        seatsId: result[j].stable,
        seatname: result[j].seatname,
        availability: result[j].bstatus,
        Ename: result[j].Ename,
      });
      ...
      for (let l = 0; l < tables_id.length; l++) {
        for (let h = 0; h < seats_id.length; h++) {
          if (tables_data[l].tableId == seats_data[h].tableId) {
            seats.push({
              date: seats_data[h].date != null ? seats_data[h].date : "",
    ...
    obj["availability"].push({
      sucess: true,
      message: "sucessfully fetch a data",
      wings: wings,
    });
